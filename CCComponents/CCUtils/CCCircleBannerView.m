...
- (void)configUI {
    ...
    UIScrollView *bgScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(16/4*3, 0, self.width-16/4*3*2, self.height)];
...
- (void)reloadItemView {
    ...
    for (int i =0; i<(_dataSource.count ?_dataSource.count :1); i++) {
        ...
        if (i == 0) {
            viewWidth = self.width-16*2;
        }
        space = self.width - viewWidth;
        image.layer.cornerRadius = 5.0f;
        width = viewWidth + space/4;
        [image setFrame:CGRectMake(viewWidth*i+space/4*i+16/4, 10, viewWidth, self.height-10-20)];
        CGFloat y = i * width;
    ...
    [_bgScrollView setContentSize:CGSizeMake((self.width-16/4*3*2)*(_dataSource.count?_dataSource.count:1), 0)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    _scrollHandle(scrollView.contentOffset.x, (self.width-16/4*3*2));
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    NSInteger index = scrollView.contentOffset.x/(self.width-16/4*3*2);
    _handle(index);
}
